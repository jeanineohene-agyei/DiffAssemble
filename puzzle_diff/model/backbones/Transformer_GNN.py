...
class Transformer_GNN(nn.Module):
    def __init__(self, input_size, hidden_dim, heads, output_size, n_layers=4, edge_dim=2,) -> None:
        super().__init__()

        self.module_list = nn.ModuleList(
            [TransformerConv(input_size, out_channels=hidden_dim // heads, heads=heads, edge_dim=edge_dim)]
            + [
                TransformerConv(
                    hidden_dim, out_channels=hidden_dim // heads, heads=heads, edge_dim=edge_dim
                )
    ...
    def forward(self, x, edge_index, edge_attr=None, move_to_cpu=False, batch=None, *args):
        attentions = []
